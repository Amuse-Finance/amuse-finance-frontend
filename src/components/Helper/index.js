...
const getStakedHistory = async (web3, user, amusedVault) => {
    try {
        ...
        for(let i = startBlock; i <= _endBlock; i = i + 10000) {
            const _step = i + 10000;
            const _result = await amusedVault.getPastEvents("UNSTKAKE", { fromBlock: i, toBlock: _step });
            _tempData = [..._tempData, ..._result]
        }

        _tempData = _tempData.filter(item => web3.utils.toChecksumAddress(item.returnValues.user) === web3.utils.toChecksumAddress(user));
        _tempData = _tempData.map(item => {
            const { blockNumber, returnValues, transactionHash: hash } = item;
